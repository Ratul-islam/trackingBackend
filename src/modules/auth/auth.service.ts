...
export async function verifyUserOTP(email: string, code: string, type: 'EMAIL_VERIFICATION' | 'PASSWORD_RESET') {
  const user = await getUserBy({email});
...
export const resetPassword = async (email: string, code: string, newPassword: string) => {
  ...
  await verifyOTP(user._id as Types.ObjectId, code, 'PASSWORD_RESET')
